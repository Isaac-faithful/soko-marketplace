...
class PaystackProvider{
 ...
 initializePayment({email,amount,currency,reference,callbackUrl,metadata,subaccount,split,transactionCharge,bearer}){return this.request("POST","/transaction/initialize",{email,amount:String(this.toSubunit(amount)),currency,reference,...(callbackUrl?{callback_url:callbackUrl}:{}),channels:this.channels(currency),metadata:JSON.stringify(metadata||{}),...(subaccount?{subaccount}:{}),...(transactionCharge!==undefined?{transaction_charge:this.toSubunit(transactionCharge)}:{}),...(split?{split:{...split,subaccounts:split.subaccounts.map(item=>({...item,share:this.toSubunit(item.share)}))}}:{}),...(bearer?{bearer}: {})})}
